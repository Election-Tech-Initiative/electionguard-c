...
KeyCeremony_Coordinator_new(uint32_t num_trustees, uint32_t threshold)
{
    ...
    if (result.status == KEYCEREMONY_COORDINATOR_SUCCESS)
    {
        ...
        {
            Crypto_public_key_new(&result.coordinator->public_keys[i],
                                  threshold);
            for (uint32_t j = 0; j < num_trustees; j++)
            {
                Crypto_encrypted_key_share_init(
                    &result.coordinator->shares[i][j]);
            }
            Crypto_rsa_public_key_new(&result.coordinator->rsa_public_keys[i]);
        }
    }
    return result;
}
...
{
    for (uint32_t i = 0; i < c->threshold; i++)
    {
        Crypto_public_key_free(&c->public_keys[i], c->threshold);
        for (uint32_t j = 0; j < c->num_trustees; j++)
        {
            Crypto_encrypted_key_share_free(&c->shares[i][j]);
        }
        Crypto_rsa_public_key_free(&c->rsa_public_keys[i]);
    }
    free(c);
}
    ...
    KeyCeremony_Coordinator c, struct key_generated_message message)
{
    ...
    if (status == KEYCEREMONY_COORDINATOR_SUCCESS)
    {
        ...
        //the hash
        if(!Crypto_check_keypair_proof(message_rep.public_key, NULL))
        {
    ...
    // Mark message->trustee_index as seen and store the public key and rsa key
    if (status == KEYCEREMONY_COORDINATOR_SUCCESS)
    {
        ...
        Crypto_public_key_free(&message_rep.public_key, c->threshold);
        Crypto_rsa_public_key_copy(
            &c->rsa_public_keys[message_rep.trustee_index],
            &message_rep.rsa_public_key);
    ...
    if (result.status == KEYCEREMONY_COORDINATOR_SUCCESS)
    {
        ...
        for (uint32_t i = 0; i < c->num_trustees; i++)
        {
            ...
            Crypto_public_key_copy(&message_rep.public_keys[i],
                                   &c->public_keys[i]);
            Crypto_rsa_public_key_new(&message_rep.rsa_public_keys[i]);
            Crypto_rsa_public_key_copy(&message_rep.rsa_public_keys[i],
                                       &c->rsa_public_keys[i]);
        }
    ...
    KeyCeremony_Coordinator c, struct shares_generated_message message)
{
    ...
    for (uint32_t i = 0; i < c->num_trustees; i++)
    {
        Crypto_encrypted_key_share_init(&message_rep.shares[i]);
    }
    {
        struct serialize_state state = {
            .status = SERIALIZE_STATE_READING,
    ...
    for (uint32_t i = 0; i < c->num_trustees; i++)
    {
        Crypto_encrypted_key_share_free(&message_rep.shares[i]);
    }
...
KeyCeremony_Coordinator_all_shares_received(KeyCeremony_Coordinator c)
{
    ...
    if (result.status == KEYCEREMONY_COORDINATOR_SUCCESS)
    {
        ...
        for (uint32_t i = 0; i < c->num_trustees; i++)
        {
            for (uint32_t j = 0; j < c->num_trustees; j++)
            {
                Crypto_encrypted_key_share_init(&message_rep.shares[i][j]);
